...
N_RUNS = 3
MODELS = ["conch", "uni", "uni2h", "clipvitbasepatch32"]
...
def yield_single_aug(image_size, jitter_strength):
    ...
    names = ["crop", "h_flip", "jitter", "gauss"]
    for name, aug in zip(names, augmentation):
        yield name, transforms.Compose([aug])
...
def fit(model_name, embed, dataset_name="tcga_crc_msi", id_n=10**4):
    ...
    for aug_name, aug in yield_single_aug(image_size=224, jitter_strength=1):
        transform_ = transforms.Compose([aug, preprocessing])
        train_ = PatchDataset(
            data["train"]["images"],
            data["train"]["labels"],
...
if __name__ == "__main__":
    ...
    for dataset, model in itertools.product(DEV_DATASETS, MODELS):
        fit(model_name=model, embed=EMBED[model], dataset_name=dataset, id_n=10**3)
